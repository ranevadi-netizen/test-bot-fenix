...
def show_log_routes(chat_id):
    ...
    for index, (route_id, route) in enumerate(ROUTES.items(), start=1):
        lines.append(
            f"\n{index}. {route['name']}\n"
            f"   Назначен: {route_assignment_text(route)}"
        )

    lines.append(
        "\nВыберите маршрут для назначения водителя "
        "или нажмите «➕ Добавить маршрут»."
    )
    set_workspace(chat_id, "\n".join(lines), routes_admin_keyboard())
...
def show_log_drivers(chat_id):
    blocks = [driver_card(driver_id) for driver_id in DRIVERS]

    set_workspace(
        chat_id,
        "🚚 Водители\n\n"
        + "\n\n".join(blocks)
        + "\n\nВыберите водителя или нажмите «➕ Добавить водителя».",
...
def show_log_today(chat_id):
    ...
    set_workspace(
        chat_id,
        "📅 Сегодня\n\n"
        + ("\n\n".join(assigned) if assigned else "Назначенных маршрутов пока нет."),
        [],
    )
...
def start_add_driver(chat_id):
    ...
    set_workspace(
        chat_id,
        "➕ Новый водитель\n\nВведите имя и фамилию одним сообщением.",
        [],
    )


def start_add_route(chat_id):
    session(chat_id)["wizard"] = {
        "type": "add_route",
        "step": "name",
        "data": {},
...
def handle_wizard_text(chat_id, text):
    ...
    if wizard["type"] == "add_driver":
        if wizard["step"] == "name":
            ...
            wizard["step"] = "plate"
            set_workspace(chat_id, "Введите госномер машины, например А123ВС198.", [])
            return True

        if wizard["step"] == "plate":
            wizard["data"]["plate"] = text.upper()
            wizard["step"] = "truck"
            set_workspace(chat_id, "Введите марку/модель машины, например Volvo FH.", [])
            return True

        if wizard["step"] == "truck":
            ...
            set_workspace(
                chat_id,
                "✅ Водитель добавлен\n\n"
                f"{DRIVERS[driver_id]['name']}\n"
                f"{DRIVERS[driver_id]['truck']} · {DRIVERS[driver_id]['plate']}",
                drivers_admin_keyboard(),
            )
            return True
    ...
    if wizard["type"] == "add_route":
        if wizard["step"] == "name":
            ...
            set_workspace(
                chat_id,
                "Теперь пришлите точки маршрута одним сообщением.\n"
        ...
        if wizard["step"] == "points":
            points_raw = [line.strip(" •-") for line in text.splitlines() if line.strip()]
            if len(points_raw) < 2:
                # Also accept semicolon-separated input.
                points_raw = [p.strip() for p in text.split(";") if p.strip()]

            if len(points_raw) < 2:
                set_workspace(
                    chat_id,
                    "Нужно минимум 2 точки. Пришлите каждую с новой строки.",
                    [],
                )
                return True
            ...
            set_workspace(
                chat_id,
                f"✅ Маршрут добавлен: {ROUTES[route_id]['name']}\n"
                f"Точек: {len(ROUTES[route_id]['points'])}",
